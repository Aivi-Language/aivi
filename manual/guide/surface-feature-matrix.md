...
| Explicit recurrence `@|> ... <|@` | yes | no | partial | yes | Recurrence seed, start, step, and wakeup-witness kernels all compile through the standard Cranelift codegen path. Suffixed integer domain literals (e.g. `5sec`) compile as stack-allocated representational carrier values. `aivi execute` is one-shot and not a long-lived recurrence host. |

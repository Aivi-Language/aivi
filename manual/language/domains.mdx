---
title: "Domains"
description: "Domains are AIVI's mechanism for context-aware semantics. They allow the language to adapt to specific problem spaces like time, geometry, or UI styling by prov"
---
...
// The '+' operator now knows how to add `Vec2` records as vectors
...
| --- | --- | --- | --- |
| `10ms`, `1s`, `5min`, `2h` | Duration | `Duration` | [aivi.chronos.duration](/stdlib/time/duration) |
| `1d`, `2w`, `3mo`, `1y` | Calendar | `CalendarDelta` | [aivi.chronos.calendar](/stdlib/time/calendar) |
| `20deg`, `1.2rad` | Angle | `Angle` | [aivi.math](/stdlib/math/math) |
| `10l`, `5s`, `30h` | Color | `ColorDelta` | [aivi.color](/stdlib/ui/color) |

```aivi
use aivi.chronos.duration (domain Duration)

deadline = { millis: 0 } + 10min
```
...
// Use a variable/expression as the numeric part of a suffix literal:
timeout1 = (n)s
timeout2 = (n + 5)s
```
...
```aivi
// Same token, different full types — all valid in one domain body:
domain Matrix over Mat4 = {
  (*): Mat4 -> Float -> Mat4 // scalar scaling (keeps `*` for scalars)
  (×): Mat4 -> Mat4 -> Mat4 // matrix-matrix product (structural transform)
  (×): Mat4 -> Vec4 -> Vec4 // matrix-vector transform (homogeneous)
}
```
...
See also: [Operators and Context](/language/operators#114-domains-and-operator-meaning).
...
Domains may define **sigils** (see [Sigils](/language/sigils)) that produce domain-typed values. For example, the `Url` domain provides `~u(https://example.com)` and the `Path` domain provides `~path[/usr/local/bin]`. These sigils are validated at compile time and construct typed values, not raw strings.
...
The only implicit conversions in v0.1 are expected-type coercions authorized by in-scope instances (see [Types: Expected-Type Coercions](/language/expected-type-coercions)).

...
description: "Bindings are lexical and may be shadowed."
---
## 1.1 Definitions
...
formatUser = u as { id, name } => "{id}: {name}"
```
...
```aivi
{ data.user.profile as { name } } = response
```
...
```aivi
{ data: { user: { profile: p as { name } } } } = response
```

...
```aivi
val boardSize:BoardSize = { width: 12, height: 10 }   -- constant, val

sig game : Signal Game = ...    -- changes on every timer tick, sig

sig board : Signal GameBoard =  -- derived from game, also sig
    game
     |> toBoard boardSize
```

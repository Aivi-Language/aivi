---
title: "Standard Library: JSON Parsing & Validation"
description: "The aivi.json module defines parsing strategies that connect raw data sources to typed structures via the Validation Applicative."
---
...
rawJson = "{ \"id\": 1, \"name\": \"Alice\", \"age\": \"twenty\" }"

// json.parse is type-driven by the assignment
result : Validation (List DecodeError) User
...
// Using it in a do block
loadUsers = do Effect {
  // If `./users.json` contains malformed entities, the Effect short-circuits
  // and yields a `SourceError.DecodeError(errs)`!
  data <- users

  // Here `data` is guaranteed to be a valid `List User`
  pure data
}
```
...
decodeStatus : JsonValue -> Validation (List DecodeError) Status
decodeStatus = val => val match
  | JsonString "active"   => Valid Active
  | JsonString "inactive" => Valid Inactive
  | _                     => Invalid [{ path: [], message: "Expected 'active' or 'inactive'" }]
```

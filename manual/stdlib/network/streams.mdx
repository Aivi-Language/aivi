---
title: "Streams Domain"
description: "The Streams domain provides stream-oriented utilities for processing inbound and outbound data without loading everything into memory."
---
...
<!-- /quick-info -->
<Note>use aivi.net.streams</Note>

```aivi
use aivi.net.streams

// fromList creates an in-memory stream for testing and pure pipelines.
nums = streams.fromList [1, 2, 3, 4, 5]
evens = streams.filter (x => x % 2 == 0) nums
doubled = streams.map (x => x * 2) evens
```

...
fn main() {
    ...
    let entries = build_entries_from_specs(&specs_dir).unwrap_or_else(|err| {
        panic!("failed to build REPL doc index from specs: {err}");
    });

    let json = serde_json::to_string_pretty(&entries).expect("serialize REPL doc index");
    fs::write(&out_path, json).expect("write REPL doc index");
}
...
fn extract_inline_code_spans(content: &str) -> Vec<String> {
    ...
    while i < content.len() {
        ...
        spans.push(content[open + 1..close].to_string());
        i = close + 1;
    }
    spans
}

...
fn cmd_project_build(args: &[String]) -> Result<(), AiviError> {
    ...
    // Check for --cranelift flag
    let use_cranelift = cargo_args.iter().any(|a| a == "--cranelift");
    let cargo_args: Vec<String> = cargo_args.into_iter().filter(|a| a != "--cranelift").collect();
...
fn cmd_project_build_cranelift(
    ...
    let object_bytes = aivi::compile_to_object(program.clone(), cg_types, monomorph_plan)?;
    ...
    let status = cmd.current_dir(root).status()?;
    if !status.success() {
        return Err(AiviError::Cargo("cargo build (cranelift aot) failed".to_string()));
    }
...
fn main() {{
    // The AOT-compiled AIVI functions are linked from the .o file.
    // The runtime helpers (rt_*) are provided by the aivi crate.
    // For now, we use the standard cranelift JIT entry point
    // since the runtime needs the full interpreter for builtins.
    let args: Vec<String> = std::env::args().collect();
    let entry = if args.len() > 1 {{ &args[1] }} else {{ "src/main.aivi" }};
    let result = aivi::desugar_target_with_cg_types(entry);
    match result {{
        Ok((program, cg_types, monomorph_plan)) => {{
            if let Err(e) = aivi::run_cranelift_jit(program, cg_types, monomorph_plan) {{
                eprintln!("Error: {{e}}");
                std::process::exit(1);
            }}
        }}

...
fn cmd_project_build_cranelift(
    ...
    if !crate_natives.is_empty() {
        let cargo_toml_path = root.join("Cargo.toml");
        if let Err(errors) =
            aivi::native_bridge::validate_crate_deps(&cargo_toml_path, &crate_natives)
        {
            for err in &errors {
                eprintln!("{err}");
            }
            return Err(AiviError::Cargo(
                "missing crate dependencies for @native bindings".to_string(),
            ));
        }
    }
    ...
    // 3. Generate the Rust harness and crate-native bridge module
    let harness_path = gen_dir.join("src");
    std::fs::create_dir_all(&harness_path)?;
    let has_crate_natives = !crate_natives.is_empty();
    let harness_main = harness_path.join("main.rs");
    let harness_code = generate_aot_harness("aivi_program", has_crate_natives);
    std::fs::write(&harness_main, harness_code)?;

    // Generate native_bridge.rs if there are crate-native bindings
    if has_crate_natives {
        let bridge_code =
            aivi::native_bridge::generate_native_bridge_source(&crate_natives);
        std::fs::write(harness_path.join("native_bridge.rs"), bridge_code)?;
    }
...
fn generate_aot_harness(project_name: &str, has_crate_natives: bool) -> String {
    let bridge_mod = if has_crate_natives {
        "mod native_bridge;\n"
    } else {
        ""
    };
    let bridge_register = if has_crate_natives {
        "\n    // Register crate-native bridge functions before AOT entry.\n    \
         aivi::register_crate_natives_on_ctx(ctx, native_bridge::register_crate_natives);\n"
    } else {
        ""
    };
    format!(
...
fn main() {{
    ...
    let ctx = aivi::init_aot_runtime_base();{bridge_register}

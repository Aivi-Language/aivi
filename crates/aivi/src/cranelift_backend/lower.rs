...
use cranelift_codegen::ir::{self, types, AbiParam, BlockArg, Function, InstBuilder, Value};
...
pub(crate) struct LowerCtx<'a, M: Module> {
    ...
    /// JIT mode must not assume local data lands within near-relocation range.
    use_far_data_refs: bool,
...
/// Import a module function into another function with an explicit colocation flag.
///
/// Cranelift's default `declare_func_in_func` marks final/local functions as
/// colocated, which lets x86 use rel32 calls. That is appropriate for AOT
/// objects, but JIT-allocated functions in a long-lived process are not
/// guaranteed to land within rel32 range of each other.
pub(crate) fn declare_module_func_in_func(
...
/// Import a module data object into a function with an explicit colocation flag.
pub(crate) fn declare_module_data_in_func(
    module: &impl Module,
    data_id: cranelift_module::DataId,
    func: &mut Function,
    colocated: bool,
) -> ir::GlobalValue {
    let decl = module.declarations().get_data_decl(data_id);
    let user_name_ref = func.declare_imported_user_function(ir::UserExternalName {
        namespace: 1,
...
pub(crate) fn declare_helpers(module: &mut impl Module) -> Result<DeclaredHelpers, String> {
    ...
    Ok(DeclaredHelpers {
        ...
        // (ctx, name_ptr, name_len, path_ptr, path_len, start_line, start_col, end_line, end_col, kind) -> void
        rt_enter_fn: decl!(
            "rt_enter_fn",
...
impl<'a, M: Module> LowerCtx<'a, M> {
    ...
    fn embed_str(&mut self, builder: &mut FunctionBuilder<'_>, s: &[u8]) -> (Value, Value) {
        ...
        *self.str_counter += 1;
        ...
        let gv = if self.use_far_data_refs {
            declare_module_data_in_func(self.module, data_id, builder.func, false)
        } else {
    ...
    pub(crate) fn emit_enter_fn(
        ...
        let (path_ptr, path_len, start_line, start_col, end_line, end_col, kind) =
            if let Some(location) = location {
                let (path_ptr, path_len) = self.embed_str(builder, location.path.as_bytes());
                (
                    path_ptr,
                    path_len,
                    builder.ins().iconst(PTR, location.span.start.line as i64),
                    builder.ins().iconst(PTR, location.span.start.column as i64),
                    builder.ins().iconst(PTR, location.span.end.line as i64),
                    builder.ins().iconst(PTR, location.span.end.column as i64),
                    builder.ins().iconst(PTR, location.source_kind.as_i64()),
                )
            } else {
                (

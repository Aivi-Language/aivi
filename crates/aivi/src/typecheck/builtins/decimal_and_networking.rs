...
pub(super) fn register(checker: &mut TypeChecker, env: &mut TypeEnv) {
    ...
    // __openapi_call : descriptor -> config -> params -> Source RestApi r
    // The result type `r` is polymorphic — the TypeSig generated by openapi_to_expr
    // constrains it to `Result { message: Text } { status: Int, ... }`.
    let oa_d = checker.fresh_var_id();
    let oa_c = checker.fresh_var_id();
    let oa_p = checker.fresh_var_id();
    let oa_r = checker.fresh_var_id();
    let oa_source_ty = Type::con("Source").app(vec![Type::con("RestApi"), Type::Var(oa_r)]);
    let oa_ty = Type::Func(

...
pub fn lower_kernel(program: HirProgram) -> Result<RustIrProgram, AiviError> {
...
fn lower_module(module: HirModule, globals: &[String]) -> Result<RustIrModule, AiviError> {
...
fn lower_def(def: HirDef, globals: &[String]) -> Result<RustIrDef, AiviError> {
...
fn lower_expr(
    ...
    Ok(match expr {
        ...
        HirExpr::LitString { id, text } => RustIrExpr::LitString { id, text },
        HirExpr::TextInterpolate { id, parts } => RustIrExpr::TextInterpolate {
            id,
            parts: parts
                .into_iter()
                .map(|part| {
                    Ok::<RustIrTextPart, AiviError>(match part {
                        HirTextPart::Text { text } => {
                            RustIrTextPart::Text { text }
                        }
                        HirTextPart::Expr { expr } => RustIrTextPart::Expr {
        ...
        HirExpr::Tuple { id, items } => RustIrExpr::Tuple {
            id,
            items: items
                .into_iter()
        ...
        HirExpr::Record { id, fields } => RustIrExpr::Record {
            id,
            fields: fields
                .into_iter()
        ...
        HirExpr::Patch { id, target, fields } => RustIrExpr::Patch {
            id,
        ...
        HirExpr::Block { .. } => unreachable!("blocks should have been desugared before RustIr lowering"),
    })
}
...
fn lower_path_segment(
    ...
    match seg {
        HirPathSegment::Field(name) => Ok(RustIrPathSegment::Field(name)),
        HirPathSegment::All => Ok(RustIrPathSegment::IndexAll),
        HirPathSegment::Index(expr) => {
            ...
            collect_unbound_vars_in_hir_expr(&expr, globals, locals, &mut bound, &mut unbound);
            if !unbound.is_empty() {

...
fn ctor_name(value: &Value) -> &str {
    match value {
        ...
        _ => panic!("expected Constructor, got {:?}", aivi_native_runtime::format_value(value)),
    }
}
...
fn as_int(value: &Value) -> i64 {
    match value {
        Value::Int(v) => *v,
        _ => panic!("expected Int, got {}", aivi_native_runtime::format_value(value)),
    }
}
...
fn as_float(value: &Value) -> f64 {
    match value {
        Value::Float(v) => *v,
        _ => panic!("expected Float, got {}", aivi_native_runtime::format_value(value)),
    }
}
...
fn as_text(value: &Value) -> &str {
    match value {
        ...
        _ => panic!("expected Text, got {}", aivi_native_runtime::format_value(value)),
    }
}
...
fn as_bool(value: &Value) -> bool {
    match value {
        Value::Bool(v) => *v,
        _ => panic!("expected Bool, got {}", aivi_native_runtime::format_value(value)),
    }
}
...
fn as_list(value: &Value) -> &[Value] {
    match value {
        ...
        _ => panic!("expected List, got {}", aivi_native_runtime::format_value(value)),
    }
}
...
fn as_tuple(value: &Value) -> &[Value] {
    match value {
        ...
        _ => panic!("expected Tuple, got {}", aivi_native_runtime::format_value(value)),
    }
}
...
mod math {
    ...
    fn sign() {
        let rt = &mut Runtime::new();
        assert_eq!(as_float(&call(rt, field(&math(), "sign"), vec![Value::Float(5.0)])), 1.0);
        assert_eq!(as_float(&call(rt, field(&math(), "sign"), vec![Value::Float(-5.0)])), -1.0);
        assert_eq!(as_float(&call(rt, field(&math(), "sign"), vec![Value::Float(0.0)])), 0.0);
    }

    #[test]
    fn min_max() {
        let rt = &mut Runtime::new();
        let min = call(rt, field(&math(), "min"), vec![Value::Float(1.0), Value::Float(2.0)]);
        assert_eq!(as_float(&min), 1.0);
        let max = call(rt, field(&math(), "max"), vec![Value::Float(1.0), Value::Float(2.0)]);
    ...
    fn clamp() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&math(), "clamp"), vec![
            Value::Float(0.0),
            Value::Float(10.0),
            Value::Float(15.0),
        ]);
    ...
    fn floor_ceil_trunc_round() {
        let rt = &mut Runtime::new();
        assert_eq!(as_float(&call(rt, field(&math(), "floor"), vec![Value::Float(2.7)])), 2.0);
        assert_eq!(as_float(&call(rt, field(&math(), "ceil"), vec![Value::Float(2.3)])), 3.0);
        assert_eq!(as_float(&call(rt, field(&math(), "trunc"), vec![Value::Float(2.9)])), 2.0);
        assert_eq!(as_float(&call(rt, field(&math(), "round"), vec![Value::Float(2.5)])), 2.0); // banker's rounding
        assert_eq!(as_float(&call(rt, field(&math(), "round"), vec![Value::Float(3.5)])), 4.0); // banker's rounding
    }

    #[test]
    fn pow_sqrt_cbrt() {
        let rt = &mut Runtime::new();
        assert!((as_float(&call(rt, field(&math(), "pow"), vec![Value::Float(2.0), Value::Float(3.0)])) - 8.0).abs() < 1e-10);
        assert!((as_float(&call(rt, field(&math(), "sqrt"), vec![Value::Float(9.0)])) - 3.0).abs() < 1e-10);
        assert!((as_float(&call(rt, field(&math(), "cbrt"), vec![Value::Float(27.0)])) - 3.0).abs() < 1e-10);
    }
    ...
    fn exp_log() {
        ...
        let ln_e = as_float(&call(rt, field(&math(), "log"), vec![Value::Float(std::f64::consts::E)]));
        assert!((ln_e - 1.0).abs() < 1e-10);

        let log10 = as_float(&call(rt, field(&math(), "log10"), vec![Value::Float(100.0)]));
    ...
    fn trig() {
        ...
        let sin_pi2 = as_float(&call(rt, field(&math(), "sin"), vec![angle(std::f64::consts::FRAC_PI_2)]));
    ...
    fn hypot() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&math(), "hypot"), vec![Value::Float(3.0), Value::Float(4.0)]);
    ...
    fn gcd_lcm() {
        let rt = &mut Runtime::new();
        assert_eq!(as_int(&call(rt, field(&math(), "gcd"), vec![Value::Int(12), Value::Int(8)])), 4);
        assert_eq!(as_int(&call(rt, field(&math(), "lcm"), vec![Value::Int(4), Value::Int(6)])), 12);
    }
    ...
    fn factorial() {
        ...
        match &result {
            ...
            other => panic!("expected BigInt, got {}", aivi_native_runtime::format_value(other)),
        }
    }
    ...
    fn divmod() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&math(), "divmod"), vec![Value::Int(7), Value::Int(3)]);
        let t = as_tuple(&r);
    ...
    fn mod_pow() {
        ...
        let r = call(rt, field(&math(), "modPow"), vec![
            Value::Int(2),
            Value::Int(10),
            Value::Int(1000),
        ]);
    ...
    fn is_finite_inf_nan() {
        let rt = &mut Runtime::new();
        assert!(as_bool(&call(rt, field(&math(), "isFinite"), vec![Value::Float(1.0)])));
        assert!(!as_bool(&call(rt, field(&math(), "isFinite"), vec![Value::Float(f64::INFINITY)])));
        assert!(as_bool(&call(rt, field(&math(), "isInf"), vec![Value::Float(f64::INFINITY)])));
        assert!(as_bool(&call(rt, field(&math(), "isNaN"), vec![Value::Float(f64::NAN)])));
        assert!(!as_bool(&call(rt, field(&math(), "isNaN"), vec![Value::Float(1.0)])));
    }
    ...
    fn copysign() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&math(), "copysign"), vec![Value::Float(5.0), Value::Float(-1.0)]);
    ...
    fn comb_perm() {
        let rt = &mut Runtime::new();
        // C(5,2) = 10
        let c = call(rt, field(&math(), "comb"), vec![Value::Int(5), Value::Int(2)]);
        match &c {
            Value::BigInt(v) => assert_eq!(**v, num_bigint::BigInt::from(10)),
            _ => panic!("expected BigInt"),
        }
        // P(5,2) = 20
        let p = call(rt, field(&math(), "perm"), vec![Value::Int(5), Value::Int(2)]);
        match &p {
    ...
    fn fmod() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&math(), "fmod"), vec![Value::Float(5.5), Value::Float(2.0)]);
    ...
    fn exp2_expm1_log1p() {
        let rt = &mut Runtime::new();
        assert!((as_float(&call(rt, field(&math(), "exp2"), vec![Value::Float(3.0)])) - 8.0).abs() < 1e-10);
    ...
    fn gcd_all_lcm_all() {
        ...
        let lcm_list = Value::List(std::sync::Arc::new(vec![
            Value::Int(4),
            Value::Int(6),
        ]));
...
mod text {
    ...
    fn length() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "length"), vec![Value::Text("hello".into())]);
    ...
    fn is_empty() {
        let rt = &mut Runtime::new();
        assert!(as_bool(&call(rt, field(&text(), "isEmpty"), vec![Value::Text("".into())])));
        assert!(!as_bool(&call(rt, field(&text(), "isEmpty"), vec![Value::Text("a".into())])));
    }

    #[test]
    fn contains() {
        let rt = &mut Runtime::new();
        assert!(as_bool(&call(rt, field(&text(), "contains"), vec![
    ...
    fn starts_ends_with() {
        let rt = &mut Runtime::new();
        assert!(as_bool(&call(rt, field(&text(), "startsWith"), vec![
            Value::Text("hello world".into()),
            Value::Text("hello".into()),
        ])));
        assert!(as_bool(&call(rt, field(&text(), "endsWith"), vec![
            Value::Text("hello world".into()),
            Value::Text("world".into()),
        ])));
    }
    ...
    fn index_of() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "indexOf"), vec![
            Value::Text("hello".into()),
            Value::Text("ll".into()),
        ]);
        ...
        let r2 = call(rt, field(&text(), "indexOf"), vec![
            Value::Text("hello".into()),
            Value::Text("xyz".into()),
        ]);
    ...
    fn slice() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "slice"), vec![
            Value::Text("hello world".into()),
            Value::Int(0),
            Value::Int(5),
        ]);
    ...
    fn split() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "split"), vec![
    ...
    fn trim() {
        let rt = &mut Runtime::new();
        assert_eq!(as_text(&call(rt, field(&text(), "trim"), vec![Value::Text("  hi  ".into())])), "hi");
        assert_eq!(as_text(&call(rt, field(&text(), "trimStart"), vec![Value::Text("  hi  ".into())])), "hi  ");
        assert_eq!(as_text(&call(rt, field(&text(), "trimEnd"), vec![Value::Text("  hi  ".into())])), "  hi");
    }

    #[test]
    fn to_lower_to_upper() {
        let rt = &mut Runtime::new();
        assert_eq!(as_text(&call(rt, field(&text(), "toLower"), vec![Value::Text("HELLO".into())])), "hello");
        assert_eq!(as_text(&call(rt, field(&text(), "toUpper"), vec![Value::Text("hello".into())])), "HELLO");
    }

    #[test]
    fn capitalize() {
        let rt = &mut Runtime::new();
        assert_eq!(as_text(&call(rt, field(&text(), "capitalize"), vec![Value::Text("hello".into())])), "Hello");
    }

    #[test]
    fn title_case() {
        let rt = &mut Runtime::new();
        assert_eq!(as_text(&call(rt, field(&text(), "titleCase"), vec![Value::Text("hello world".into())])), "Hello World");
    }

    #[test]
    fn replace() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "replace"), vec![
    ...
    fn replace_all() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "replaceAll"), vec![
            Value::Text("aababab".into()),
            Value::Text("ab".into()),
            Value::Text("X".into()),
        ]);
    ...
    fn repeat() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "repeat"), vec![
            Value::Text("ab".into()),
            Value::Int(3),
        ]);
    ...
    fn reverse() {
        let rt = &mut Runtime::new();
        assert_eq!(as_text(&call(rt, field(&text(), "reverse"), vec![Value::Text("abc".into())])), "cba");
    }
    ...
    fn concat() {
        ...
        assert_eq!(as_text(&call(rt, field(&text(), "concat"), vec![list])), "abc");
    }

    #[test]
    fn pad_start_end() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "padStart"), vec![
            Value::Text("hi".into()),
            Value::Int(5),
            Value::Text("0".into()),
        ]);
        ...
        let r2 = call(rt, field(&text(), "padEnd"), vec![
            Value::Text("hi".into()),
            Value::Int(5),
            Value::Text("0".into()),
        ]);
    ...
    fn count() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "count"), vec![
            Value::Text("abcabc".into()),
            Value::Text("abc".into()),
        ]);
    ...
    fn compare() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "compare"), vec![
            Value::Text("a".into()),
            Value::Text("b".into()),
        ]);
        assert!(as_int(&r) < 0);
    ...
    fn split_lines() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "splitLines"), vec![Value::Text("a\nb\nc".into())]);
        let items = as_list(&r);
        assert_eq!(items.len(), 3);
    }
    ...
    fn chunk() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "chunk"), vec![Value::Text("abcde".into()), Value::Int(2)]);
        let items = as_list(&r);
        assert_eq!(items.len(), 3);
    ...
    fn remove() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "remove"), vec![
            Value::Text("hello world".into()),
            Value::Text("world".into()),
        ]);
        assert_eq!(as_text(&r), "hello ");
    ...
    fn parse_int() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "parseInt"), vec![Value::Text("42".into())]);
        assert_eq!(ctor_name(&r), "Some");
        assert_eq!(as_int(&ctor_arg(&r, 0)), 42);

        let r2 = call(rt, field(&text(), "parseInt"), vec![Value::Text("abc".into())]);
        assert_eq!(ctor_name(&r2), "None");
    }
    ...
    fn parse_float() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "parseFloat"), vec![Value::Text("3.14".into())]);
        assert_eq!(ctor_name(&r), "Some");
    ...
    fn is_digit_alpha() {
        let rt = &mut Runtime::new();
        assert!(as_bool(&call(rt, field(&text(), "isDigit"), vec![Value::Text("5".into())])));
        assert!(!as_bool(&call(rt, field(&text(), "isDigit"), vec![Value::Text("a".into())])));
        assert!(as_bool(&call(rt, field(&text(), "isAlpha"), vec![Value::Text("a".into())])));
        assert!(!as_bool(&call(rt, field(&text(), "isAlpha"), vec![Value::Text("5".into())])));
    }
    ...
    fn to_bytes_from_bytes_utf8() {
        ...
        let bytes = call(rt, field(&text(), "toBytes"), vec![
            utf8.clone(),
            Value::Text("hello".into()),
        ]);
        match &bytes {
    ...
    fn normalize_nfc() {
        ...
        let r = call(rt, field(&text(), "normalizeNFC"), vec![Value::Text("e\u{0301}".into())]);
    ...
    fn last_index_of() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "lastIndexOf"), vec![
            Value::Text("abcabc".into()),
            Value::Text("abc".into()),
        ]);
        assert_eq!(ctor_name(&r), "Some");
    ...
    fn case_fold() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&text(), "caseFold"), vec![Value::Text("HELLO".into())]);
        assert_eq!(as_text(&r), "hello");
    }
...
mod collections {
    ...
    mod map {
        ...
        fn insert_get_has() {
            let rt = &mut Runtime::new();
            let empty = field(&map_rec(), "empty");
            let m = call(rt, field(&map_rec(), "insert"), vec![
                Value::Text("key".into()),
                Value::Int(42),
                empty,
            ]);
            let got = call(rt, field(&map_rec(), "get"), vec![Value::Text("key".into()), m.clone()]);
            assert_eq!(ctor_name(&got), "Some");
            assert_eq!(as_int(&ctor_arg(&got, 0)), 42);

            let has = call(rt, field(&map_rec(), "has"), vec![Value::Text("key".into()), m.clone()]);
            assert!(as_bool(&has));

            let missing = call(rt, field(&map_rec(), "get"), vec![Value::Text("nope".into()), m]);
        ...
        fn remove() {
            ...
            let removed = call(rt, field(&map_rec(), "remove"), vec![Value::Text("a".into()), m]);
        ...
        fn union() {
            let rt = &mut Runtime::new();
            let empty = field(&map_rec(), "empty");
            let m1 = call(rt, field(&map_rec(), "insert"), vec![
                Value::Text("a".into()),
                Value::Int(1),
                empty.clone(),
            ]);
            let m2 = call(rt, field(&map_rec(), "insert"), vec![
                Value::Text("b".into()),
                Value::Int(2),
                empty,
            ]);
    ...
    mod set {
        ...
        fn insert_has_remove() {
            ...
            assert!(as_bool(&call(rt, field(&set_rec(), "has"), vec![Value::Int(1), s.clone()])));
        ...
        fn union_intersection_difference() {
            let rt = &mut Runtime::new();
            let empty = field(&set_rec(), "empty");
            let s1 = call(rt, field(&set_rec(), "insert"), vec![Value::Int(1), empty.clone()]);
            let s1 = call(rt, field(&set_rec(), "insert"), vec![Value::Int(2), s1]);
            let s2 = call(rt, field(&set_rec(), "insert"), vec![Value::Int(2), empty.clone()]);
            ...
            let inter = call(rt, field(&set_rec(), "intersection"), vec![s1.clone(), s2.clone()]);
    ...
    mod queue {
        ...
        fn enqueue_dequeue_peek() {
            ...
            let q = call(rt, field(&queue_rec(), "enqueue"), vec![Value::Int(1), empty]);
    ...
    mod deque {
        ...
        fn push_pop_front_back() {
            ...
            let d = call(rt, field(&deque_rec(), "pushFront"), vec![Value::Int(1), empty]);
...
mod crypto {
    ...
    fn sha256() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&crypto(), "sha256"), vec![Value::Text("hello".into())]);
        let hex = as_text(&r);
...
mod color {
    ...
    fn adjust_lightness() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&color(), "adjustLightness"), vec![rgb(128, 128, 128), Value::Int(10)]);
        // Should be lighter
    ...
    fn adjust_saturation() {
        ...
        let r = call(rt, field(&color(), "adjustSaturation"), vec![rgb(255, 0, 0), Value::Int(-50)]);
    ...
    fn adjust_hue() {
        ...
        let r = call(rt, field(&color(), "adjustHue"), vec![rgb(255, 0, 0), Value::Int(120)]);
        let g_val = as_int(&field(&r, "g"));
        assert!(g_val > 200);
    }
}
...
mod calendar {
    ...
    fn is_leap_year() {
        let rt = &mut Runtime::new();
        assert!(as_bool(&call(rt, field(&calendar(), "isLeapYear"), vec![date(2024, 1, 1)])));
        assert!(!as_bool(&call(rt, field(&calendar(), "isLeapYear"), vec![date(2023, 1, 1)])));
        assert!(as_bool(&call(rt, field(&calendar(), "isLeapYear"), vec![date(2000, 1, 1)])));
        assert!(!as_bool(&call(rt, field(&calendar(), "isLeapYear"), vec![date(1900, 1, 1)])));
    }

    #[test]
    fn days_in_month() {
        let rt = &mut Runtime::new();
        assert_eq!(as_int(&call(rt, field(&calendar(), "daysInMonth"), vec![date(2024, 2, 1)])), 29);
        assert_eq!(as_int(&call(rt, field(&calendar(), "daysInMonth"), vec![date(2023, 2, 1)])), 28);
        assert_eq!(as_int(&call(rt, field(&calendar(), "daysInMonth"), vec![date(2024, 1, 1)])), 31);
    }

    #[test]
    fn end_of_month() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&calendar(), "endOfMonth"), vec![date(2024, 2, 15)]);
    ...
    fn add_days() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&calendar(), "addDays"), vec![date(2024, 1, 30), Value::Int(5)]);
    ...
    fn add_months() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&calendar(), "addMonths"), vec![date(2024, 1, 31), Value::Int(1)]);
        assert_eq!(as_int(&field(&r, "month")), 2);
    ...
    fn add_years() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&calendar(), "addYears"), vec![date(2024, 2, 29), Value::Int(1)]);
...
mod regex_tests {
    ...
    fn compile_ok() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&regex(), "compile"), vec![Value::Text(r"\d+".into())]);
    ...
    fn compile_err() {
        let rt = &mut Runtime::new();
        let r = call(rt, field(&regex(), "compile"), vec![Value::Text("[invalid".into())]);
    ...
    fn test_match() {
        let rt = &mut Runtime::new();
        let compiled = call(rt, field(&regex(), "compile"), vec![Value::Text(r"\d+".into())]);
        let rx = ctor_arg(&compiled, 0);

        assert!(as_bool(&call(rt, field(&regex(), "test"), vec![rx.clone(), Value::Text("abc123def".into())])));
        assert!(!as_bool(&call(rt, field(&regex(), "test"), vec![rx, Value::Text("abcdef".into())])));
    }

    #[test]
    fn match_captures() {
        let rt = &mut Runtime::new();
        let compiled = call(rt, field(&regex(), "compile"), vec![Value::Text(r"(\d+)-(\w+)".into())]);
        let rx = ctor_arg(&compiled, 0);
    ...
    fn find_all() {
        ...
        let results = call(rt, field(&regex(), "findAll"), vec![rx, Value::Text("1 and 22 and 333".into())]);
    ...
    fn split() {
        let rt = &mut Runtime::new();
        let compiled = call(rt, field(&regex(), "compile"), vec![Value::Text(r"\s+".into())]);
        let rx = ctor_arg(&compiled, 0);

        let parts = call(rt, field(&regex(), "split"), vec![rx, Value::Text("a  b   c".into())]);
    ...
    fn replace() {
        ...
        let result = call(rt, field(&regex(), "replace"), vec![
            rx,
    ...
    fn replace_all() {
        ...
        let result = call(rt, field(&regex(), "replaceAll"), vec![
            rx,
            Value::Text("abc 123 def 456".into()),
            Value::Text("NUM".into()),
        ]);
    ...
    fn matches_multiple() {
        let rt = &mut Runtime::new();
        let compiled = call(rt, field(&regex(), "compile"), vec![Value::Text(r"(\w+)@(\w+)".into())]);
        let rx = ctor_arg(&compiled, 0);

        let results = call(rt, field(&regex(), "matches"), vec![rx, Value::Text("a@b c@d".into())]);
...
mod number {
    ...
    fn decimal_round() {
        let rt = &mut Runtime::new();
        let d = call(rt, field(&decimal(), "fromFloat"), vec![Value::Float(3.14159)]);
...
mod core_builtins {
    ...
    fn bind_chaining() {
        ...
        let add_one = Value::Closure(std::sync::Arc::new(aivi_native_runtime::ClosureValue {
            func: std::sync::Arc::new(|arg, rt| {
                let n = match arg {
                    Value::Int(n) => n,
                    _ => return Err(aivi_native_runtime::RuntimeError::Message("expected Int".into())),
                };
                let inner = rt.call(
    ...
    fn map_option() {
        let rt = &mut Runtime::new();
        let some = Value::Constructor { name: "Some".into(), args: vec![Value::Int(5)] };
        let none = Value::Constructor { name: "None".into(), args: vec![] };
        let add_one = Value::Closure(std::sync::Arc::new(aivi_native_runtime::ClosureValue {
            func: std::sync::Arc::new(|arg, _| match arg {
                Value::Int(n) => Ok(Value::Int(n + 1)),
                _ => Err(aivi_native_runtime::RuntimeError::Message("expected Int".into())),
            }),
        }));
    ...
    fn chain_list() {
        ...
        let dup = Value::Closure(std::sync::Arc::new(aivi_native_runtime::ClosureValue {
            func: std::sync::Arc::new(|arg, _| {
                ...
                Ok(Value::List(std::sync::Arc::new(vec![Value::Int(n), Value::Int(n)])))
            }),
        }));
...
mod key_value {
    ...
    fn round_trip_unit() {
        ...
        assert!(aivi_native_runtime::values_equal(&kv.to_value(), &Value::Unit));
    }

    #[test]
    fn round_trip_int() {
        let kv = KeyValue::try_from_value(&Value::Int(42)).unwrap();
        assert!(aivi_native_runtime::values_equal(&kv.to_value(), &Value::Int(42)));
    }
    ...
    fn round_trip_float() {
        ...
        assert!(aivi_native_runtime::values_equal(&kv.to_value(), &Value::Float(1.5)));
    }

    #[test]
    fn round_trip_bool() {
        let kv = KeyValue::try_from_value(&Value::Bool(true)).unwrap();
        assert!(aivi_native_runtime::values_equal(&kv.to_value(), &Value::Bool(true)));
    }

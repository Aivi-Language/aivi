...
fn server_html_handle_event(
    ...
    let msg = match handler {
        ...
        ServerHtmlHandler::FnText(f) => {
            let value =
                server_html_extract_text_value(kind, &ev.payload).map_err(RuntimeError::Message)?;
...
fn server_html_render_vnode(
    vnode: &Value,
    node_id: &str,
) -> (String, HashMap<i64, (ServerHtmlEventKind, ServerHtmlHandler)>) {
        ...
        Value::Constructor { name, args } if name == "Element" && args.len() == 3 => {
            ...
            if let Value::List(items) = children_value {
                for (idx, child) in items.iter().enumerate() {
                    ...
                    children_html
                        .push_str(&server_html_render_vnode_inner(child, &child_id, None, state));
                }
            }
            format!(
        ...
        match attr {
            ...
            Value::Constructor { name, args } if name == "OnInput" && args.len() == 1 => {
                server_html_add_handler(
                    ...
                    state,
                    node_id,
                    ServerHtmlEventKind::Input,
                    ServerHtmlHandler::FnText(args[0].clone()),
                );
            }
            ...
            Value::Constructor { name, args } if name == "OnPointerMove" && args.len() == 1 => {
                server_html_add_handler(
                    ...
                    state,
                    node_id,
                    ServerHtmlEventKind::PointerMove,
                    ServerHtmlHandler::FnRecord(args[0].clone()),
                );
            }
...
fn server_html_pointer_event_value(payload: &serde_json::Value) -> Result<Value, String> {
    ...
    fields.insert(
        ...
        Value::Text(payload.get("pointerType").and_then(|v| v.as_str()).unwrap_or("").to_string()),
    );
    fields.insert(
        "button".to_string(),
        Value::Int(payload.get("button").and_then(|v| v.as_i64()).unwrap_or(0)),

...
mod linux_impl {
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                rt.block_on(async {
                    ...
                    let dbus_menu = DbusMenuObject {
                        tray_state: state_clone,
                    };
    ...
    impl DbusMenuObject {
        ...
        fn get_property(&self, _id: i32, _name: &str) -> zbus::zvariant::OwnedValue {
    ...
    pub(super) fn tray_icon_new(icon: &str, tooltip: &str) -> Result<i64, Gtk4Error> {
        ...
        if let Err(e) = spawn_sni_tray(state.clone()) {
            return Err(Gtk4Error::new(format!("gtk4.trayIconNew: {e}")));
        }
    ...
    pub(super) fn tray_icon_set_tooltip(id: i64, tooltip: &str) -> Result<(), Gtk4Error> {
        let arc = GTK_STATE.with(|s| s.borrow().tray_handles.get(&id).cloned());
        if let Some(arc) = arc {
            if let Ok(mut ts) = arc.lock() {
                ts.tooltip = tooltip.to_string();
            }
        }
        Ok(())
    }

    pub(super) fn tray_icon_set_visible(id: i64, visible: bool) -> Result<(), Gtk4Error> {
        let arc = GTK_STATE.with(|s| s.borrow().tray_handles.get(&id).cloned());
        if let Some(arc) = arc {
            if let Ok(mut ts) = arc.lock() {
                ts.visible = visible;
    ...
    pub(super) fn tray_icon_set_menu_items(
        id: i64,
        items: Vec<(String, String)>,
    ) -> Result<(), Gtk4Error> {
        GTK_STATE.with(|s| {
            let s = s.borrow();
            if let Some(handle) = s.tray_handles.get(&id) {
    ...
    pub(super) fn os_set_badge_count(_app_id: i64, count: i64) -> Result<(), Gtk4Error> {
        let arc = GTK_STATE.with(|s| s.borrow().tray_handles.values().last().cloned());
        if let Some(arc) = arc {
            if let Ok(mut ts) = arc.lock() {
...
delegate!(tray_icon_new(icon: &str, tooltip: &str) -> Result<i64, Gtk4Error>);
delegate!(tray_icon_set_tooltip(id: i64, tooltip: &str) -> Result<(), Gtk4Error>);
delegate!(tray_icon_set_visible(id: i64, visible: bool) -> Result<(), Gtk4Error>);
delegate!(tray_icon_set_menu_items(id: i64, items: Vec<(String, String)>) -> Result<(), Gtk4Error>);

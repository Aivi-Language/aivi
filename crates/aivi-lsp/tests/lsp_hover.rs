...
#[test]
fn hover_on_markup_value_name_returns_widget_type() {
...
#[test]
fn hover_on_func_declaration_uses_func_kind_label() {
    // "type Int -> Int\nfunc id = x =>\n    x\n"
    // 'id' is on line 1, character 5
    let text = "type Int -> Int\nfunc id = x =>\n    x\n";
    let (state, uri) = open_inline("hover-func.aivi", text);
    let markup = hover_markup(hover(hover_params(uri, 1, 5), state));
...
#[test]
fn hover_on_signal_declaration_uses_signal_kind_label() {
    // signal tick = 0
    // 'tick' is on line 0, character 7
    let text = "signal tick = 0\n";
    let (state, uri) = open_inline("hover-signal.aivi", text);
    let markup = hover_markup(hover(hover_params(uri, 0, 7), state));
...
#[test]
fn hover_colon_separated_from_type_detail() {
...
#[test]
fn hover_on_reference_site_uses_inferred_declaration_type() {
    let text = "value answer = 42\nvalue total = answer\n";
    let (state, uri) = open_inline("hover-reference.aivi", text);
    let markup = hover_markup(hover(hover_params(uri, 1, 14), state));
...
#[test]
fn hover_on_mismatched_annotation_mentions_declared_type() {
...
#[test]
fn hover_on_from_source_survives_multibyte_text_in_reversi() {
...
#[test]
fn hover_survives_box_drawing_comments_before_binary_exprs() {
    ...
    let markup = hover_markup(hover(
        hover_params(uri, position.line, position.character),
        state,
    ));

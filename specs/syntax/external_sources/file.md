...
At the type level, file-source failures surface as [`SourceError File`](../external_sources.md#128-sourceerror). The decode branch carries path-aware [`aivi.validation.DecodeError`](../../stdlib/core/validation.md#5-decodeerror-adt) values, which format into messages that point to both:

...
The runtime provides a default implementation that writes one JSON log entry per line. `trace`, `debug`, and `info` go to standard output, while `warn` and `error` go to standard error. Route logs through ordinary bindings or surrounding runtime configuration when you need a different sink.

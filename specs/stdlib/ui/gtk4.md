...
`gtkApp` is the single blessed entry point for GTK applications in AIVI. The user provides a configuration record and `gtkApp` handles init, startup, window creation, event ingestion, reconciliation, and the currently implemented command/subscription subset from [GTK App Architecture](/stdlib/ui/app_architecture):
...
`onStart` is the blessed place for one-time startup work such as app CSS or action registration. Repeating timers, background work, and external feeds belong to the command/subscription model; steady-state application behavior still flows through `Msg` and `update`. `gtkSetInterval` remains available only as a low-level escape hatch for manual loops and legacy code.
